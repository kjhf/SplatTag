...
namespace SplatTagConsole
{
  internal class ConsoleController
  {
    ...
    private readonly HashSet<string> errorMessagesReported;
    private readonly SplatTagController splatTagController;

    static ConsoleController()
    {
      command = new Command("Slapp", "Slapp Query");
    ...
    /// <summary>
    /// Initialise the <see cref="SplatTagController"/> if it has not yet.
    /// NOte that the <see cref="Handle(string[])"/> method will call this if required anyway.
    /// </summary>
    ...
    internal async Task Handle(string[] args)
    {
      // If we aren't rebuilding or patching, initialise the controller here
    ...
    private void HandleCommandLineQuery(
      string? b64,
      string? query,
      string? slappId,
      bool exactCase,
